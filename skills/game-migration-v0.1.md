...
- Source: Unity/C# turn-based roguelike or similar game (e.g. `DreamDeveloperGame-Crossover`)
- Framework: Rust workspace providing generic game systems (e.g. `framework_rules`, `framework_combat`, `framework_progression`, `framework_ai`)
- Engine: Rust workspace providing runtime infrastructure (e.g. `WorldEngine` with `simulation_core`, `content_registry`, `contracts`, `presentation_projector`)
- Target: Consumer game crate using framework public APIs + engine runtime (e.g. `DDGC_newArch`)

## Core Rules

### 1. Semantic Parity Over Structural Translation

Migrate for **behavioral equivalence**, not code-line correspondence. The internal implementation may differ entirely (different class hierarchy, data structures, dispatch mechanism) as long as every **player-observable behavior** produces identical outcomes for identical inputs.
...
### 3. Blocker Classification System

Every migration obstacle must be classified before resolution:
...
**Backflow rejection criteria:** A patch to framework/engine crates is only justified if the capability would benefit *any* consumer, not just the source game. Game-specific mechanics (e.g. "stress", "chaos", "limit per turn") are always game-gaps.
...
### 4. Headless-First, Deterministic Verification
...
### 6. Fixture-Driven Content Migration
...
- [ ] All parity tests for the batch pass
- [ ] No game-specific content in framework or engine crates
- [ ] All blockers in the batch classified and resolved or deferred
- [ ] CHANGELOG entry appended

## Version History

- **v0.1** (2026-04-25): Initial rule set derived from DDGC → WorldEngine + glowing-fishstick-framwork migration.
